...
export default function MediaSections({ webcamShows, episodes }) {
  return (
    <>
      {/* Live Webcam Shows */}
      <div className="w-full pt-7 border-b border-[#1a1a1a]">
        ...
        >
          {webcamShows.map((show) => (
            <>
              <div
                key={show.id}
                className="relative flex-shrink-0 sm:min-w-[150px] sm:h-[180px] min-w-[100px] h-[130px] rounded-lg overflow-hidden group"
              >
                {/* Background Image */}
                <Image
                  src={show.image}
          ...
          <div className="relative flex-shrink-0 sm:min-w-[150px] sm:h-[180px] min-w-[100px] h-[130px] rounded-lg border border-[#d4a574] bg-[#1a1a1a]/70 flex flex-col items-center justify-center cursor-pointer hover:bg-[#2a2a2a] transition-all duration-300 group">
            <ChevronRightCircle
              size={40}
              className="text-[#d4a574] mb-2 group-hover:scale-110 transition"
            />
      ...
      {/* Diamond Escorts */}
      <div className="w-full sm:py-7 py-3 border-b border-[#1a1a1a]">
        <h2 className="text-lg mb-4 text-white font-medium flex items-center gap-1">
        ...
        >
          {episodes.map((episode) => (
            <div
              key={episode.id}
              className="relative flex-shrink-0 sm:min-w-[245px] sm:h-[300px] min-w-[178px] h-[250px] rounded-lg overflow-hidden group border border-[#D4A574] cursor-pointer"
            >
              {/* Profile Image */}
              <Image
                src={episode.image}
...
rounded-sm border border-white/10 shadow-md"
          ...
          <div className="relative flex-shrink-0 sm:min-w-[245px] sm:h-[300px] min-w-[178px] h-[250px] rounded-lg border border-[#d4a574] bg-[#1a1a1a]/70 flex flex-col items-center justify-center cursor-pointer hover:bg-[#2a2a2a] transition-all duration-300 group">
            <ChevronRightCircle
              size={40}
              className="text-[#d4a574] mb-2 group-hover:scale-110 transition"
            />
